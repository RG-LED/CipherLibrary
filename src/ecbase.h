...
// ---- SECP256K1 ----
template<>
struct EcTraits<EcCurve::SECP256K1> {
    static constexpr SIZE_T SCALAR_SIZE = 32;

    using CField = CFeSecp256k1;
    using CScalar = CScalarSecp256k1;
};
...
public:
    ...
    VOID cswap(CEcPoint<C> & other, BOOL swap)
    {
    ...
    static VOID Double(CEcPoint<C> & out, const CEcPoint<C> & p1)
    {
    ...
    static VOID Add(CEcPoint<C> & out, const CEcPoint<C> & p1, const CEcPoint<C> & p2)
    {
        if ( p1.IsInfinity() )
        {
            out = p2;
    ...
    static VOID ScalarMul(CEcPoint<C> & out, const CEcPoint<C> & p, const CScalar & k)
    {
        CEcPoint<C> R0;
        CEcPoint<C> R1;
        R0.SetInfinity();
        ...
        for ( INT32 i = SCALAR_SIZE * 8 - 1; i >= 0; i-- )
        {
            ...
            R0.cswap(R1, swap);
            CEcPoint<C>::Add(R1, R0, R1);
            CEcPoint<C>::Double(R0, R0);
            R0.cswap(R1, swap);
        }
        out = R0;
    }
